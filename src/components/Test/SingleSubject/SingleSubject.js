...
import FirebaseWrite from './Game/FirebaseWrite';
import FirebaseRead from './Game/FirebaseRead';
...
<>
 <div className = 'mtop' ></div>
  <div>
     ...
     {!join&&<input type = 'text' placeholder = 'Room ID' value = {id} onChange = {(e)=>setID(e.target.value)}></input>}
     {!join&&<button onClick = {handleJoin}>Join</button>}
     <br></br><p></p>
         {!join&&<button onClick = {createRoom}>Create Room</button>}
        {join&&<FetchRoom setSub = {setCloudSub} setHost = {setMeHost} leave = {leave} start = {startGame} id = {id} back = {back}/>}
        {create&&<div className = 'w3-panel w3-pale-green'><b>Room Creted Successfully</b></div>}
